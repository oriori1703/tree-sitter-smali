fn main() {
    ...
    c_config.compile("tree-sitter-smali");

    println!("cargo:rustc-check-cfg=cfg(with_folds_query)");
    if !"queries/folds.scm".is_empty() && std::path::Path::new("queries/folds.scm").exists() {
        println!("cargo:rustc-cfg=with_folds_query");
    }
    println!("cargo:rustc-check-cfg=cfg(with_indents_query)");
    if !"queries/indents.scm".is_empty() && std::path::Path::new("queries/indents.scm").exists() {
        println!("cargo:rustc-cfg=with_indents_query");
    }
